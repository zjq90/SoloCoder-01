    ...
    (0, estree_walker_1.walk)(ast, {
        enter(node, _parent) {
            ...
            if (node.type === 'VariableDeclarator' &&
                ...
                const start = node.id.loc.start;
                s.appendRight(startOffset + node.body.start + 1, ` override __$getOriginalPosition(): UTSSourceMapPosition { return new UTSSourceMapPosition("${node.id.name}", "${fileName}", ${startLine + start.line}, ${start.column + 1});} `);
            }
        },
    });
}
