...
const transformElement = (node, context) => {
    ...
    return function postTransformElement() {
        ...
        let vnodeTag = isComponent
            ...
            : `"${tag}"`;
        const isDynamicComponent = (0, shared_1.isObject)(vnodeTag) && vnodeTag.callee === compiler_core_1.RESOLVE_DYNAMIC_COMPONENT;
        let vnodeProps;
        let vnodeChildren;
        let vnodePatchFlag;
        let patchFlag = 0;
        ...
        isDynamicComponent ||
            vnodeTag === compiler_core_1.TELEPORT ||
            vnodeTag === compiler_core_1.SUSPENSE ||
            (!isComponent &&
        ...
        if (node.children.length > 0) {
            if (vnodeTag === compiler_core_1.KEEP_ALIVE) {
                ...
                if (utils_1.__DEV__ && node.children.length > 1) {
                    context.onError((0, errors_1.createCompilerError)(compiler_core_1.ErrorCodes.X_KEEP_ALIVE_INVALID_CHILDREN, {
            ...
            const shouldBuildAsSlots = isComponent &&
                ...
                vnodeTag !== compiler_core_1.TELEPORT &&
                // explained above.
                vnodeTag !== compiler_core_1.KEEP_ALIVE;
            if (shouldBuildAsSlots) {
                const { slots, hasDynamicSlots } = (0, compiler_core_1.buildSlots)(node, context);
                vnodeChildren = slots;
                if (hasDynamicSlots) {
                    patchFlag |= shared_1.PatchFlags.DYNAMIC_SLOTS;
                }
            }
            else if (node.children.length === 1 && vnodeTag !== compiler_core_1.TELEPORT) {
                ...
                if (hasDynamicTextChild &&
                    (0, compiler_core_1.getConstantType)(child, context) === compiler_core_1.ConstantTypes.NOT_CONSTANT) {
...
function resolveComponentType(node, context, ssr = false) {
    ...
    const isProp = (0, compiler_core_1.findProp)(node, 'is');
    if (isProp) {
        if (isExplicitDynamic) {
            const exp = isProp.type === compiler_core_1.NodeTypes.ATTRIBUTE
                ...
                : isProp.exp;
            if (exp) {
                return (0, compiler_core_1.createCallExpression)(context.helper(compiler_core_1.RESOLVE_DYNAMIC_COMPONENT), [
                    exp,
                ]);
            }
        }
    ...
    if (!utils_1.__BROWSER__ &&
        ...
        return (0, compiler_core_1.toValidAssetId)(tag, `component`);
    }
...
function resolveSetupReference(name, context) {
    ...
    const fromProps = checkType(compiler_core_1.BindingTypes.PROPS);
    if (fromProps) {
        return `${context.helperString(compiler_core_1.UNREF)}(${context.inline ? '__props' : '$props'}[${JSON.stringify(fromProps)}])`;
    }
}
function buildProps(node, context, props = node.props, isComponent, isDynamicComponent, ssr = false) {
        ...
        if ((0, compiler_core_1.isStaticExp)(key)) {
            ...
            if (value.type === compiler_core_1.NodeTypes.JS_CACHE_EXPRESSION ||
                ((value.type === compiler_core_1.NodeTypes.SIMPLE_EXPRESSION ||
                    ...
                    (0, compiler_core_1.getConstantType)(value, context) > 0)) {
        ...
        if (prop.type === compiler_core_1.NodeTypes.ATTRIBUTE) {
            ...
            if (name === 'ref') {
                ...
                if (!utils_1.__BROWSER__ && value && context.inline) {
                    ...
                    if (binding === compiler_core_1.BindingTypes.SETUP_LET ||
                        binding === compiler_core_1.BindingTypes.SETUP_REF ||
                        binding === compiler_core_1.BindingTypes.SETUP_MAYBE_REF) {
                        isStatic = false;
            ...
            properties.push((0, compiler_core_1.createObjectProperty)((0, compiler_core_1.createSimpleExpression)(name, true, (0, uni_cli_shared_1.getInnerRange)(loc, 0, name.length)), (0, compiler_core_1.createSimpleExpression)(value ? value.content : '', isStatic, value ? value.loc : loc)));
        }
        else {
            // directives
            ...
            if (name === 'slot') {
                if (!isComponent) {
                    context.onError((0, errors_1.createCompilerError)(compiler_core_1.ErrorCodes.X_V_SLOT_MISPLACED, loc));
                }
                continue;
            }
            ...
            if (name === 'is' ||
                (isVBind && (0, compiler_core_1.isStaticArgOf)(arg, 'is') && isComponentTag(tag))) {
                continue;
            }
            ...
            (isVBind && (0, compiler_core_1.isStaticArgOf)(arg, 'key')) ||
                // inline before-update hooks need to force block so that it is invoked
                // before children
                (isVOn && hasChildren && (0, compiler_core_1.isStaticArgOf)(arg, 'vue:before-update'))) {
                shouldUseBlock = true;
            }
            if (isVBind && (0, compiler_core_1.isStaticArgOf)(arg, 'ref') && context.scopes.vFor > 0) {
                properties.push((0, compiler_core_1.createObjectProperty)((0, compiler_core_1.createSimpleExpression)('ref_for', true), (0, compiler_core_1.createSimpleExpression)('true')));
            }
            ...
            if (!arg && (isVBind || isVOn)) {
                ...
                if (exp) {
                        ...
                        if (utils_1.__COMPAT__) {
                            ...
                            if (utils_1.__DEV__) {
                                ...
                                if (hasOverridableKeys) {
                                    (0, compiler_core_1.checkCompatEnabled)(compiler_core_1.CompilerDeprecationTypes.COMPILER_V_BIND_OBJECT_ORDER, context, loc);
                                }
                            }
                        }
                        mergeArgs.push(exp);
                    ...
                    context.onError((0, errors_1.createCompilerError)(isVBind
                        ? compiler_core_1.ErrorCodes.X_V_BIND_NO_EXPRESSION
                        : compiler_core_1.ErrorCodes.X_V_ON_NO_EXPRESSION, loc));
                }
                continue;
            }
            ...
            if (directiveTransform) {
                ...
                if (isVOn && arg && !(0, compiler_core_1.isStaticExp)(arg)) {
    ...
    if (mergeArgs.length) {
        ...
        if (mergeArgs.length > 1) {
            propsExpression = (0, compiler_core_1.createCallExpression)(context.helper(compiler_core_1.MERGE_PROPS), mergeArgs, elementLoc);
        }
        else {
    ...
    if (propsExpression) {
        switch (propsExpression.type) {
            case compiler_core_1.NodeTypes.JS_OBJECT_EXPRESSION:
                ...
                // no dynamic key
                if (!hasDynamicKey) {
                    if (classProp && !(0, compiler_core_1.isStaticExp)(classProp.value)) {
                        classProp.value = (0, compiler_core_1.createCallExpression)(context.helper(compiler_core_1.NORMALIZE_CLASS), [classProp.value]);
                    }
                    if (styleProp &&
                        ...
                        styleProp.value = (0, compiler_core_1.createCallExpression)(context.helper(compiler_core_1.NORMALIZE_STYLE), [styleProp.value]);
                    }
                }
                else {
                    // dynamic key binding, wrap with `normalizeProps`
                    propsExpression = (0, compiler_core_1.createCallExpression)(context.helper(compiler_core_1.NORMALIZE_PROPS), [propsExpression]);
                }
                break;
            ...
            default:
                // single v-bind
                propsExpression = (0, compiler_core_1.createCallExpression)(context.helper(compiler_core_1.NORMALIZE_PROPS), [
                    (0, compiler_core_1.createCallExpression)(context.helper(compiler_core_1.GUARD_REACTIVE_PROPS), [
                        propsExpression,
                    ]),
                ]);
                break;
...
function buildDirectiveArgs(dir, context) {
    ...
    if (runtime) {
        ...
        if (fromSetup) {
            ...
            dirArgs.push((0, compiler_core_1.toValidAssetId)(dir.name, `directive`));
        }
    }
    const { loc } = dir;
    if (dir.exp)
